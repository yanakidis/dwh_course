...
def process_seats(operation, before, after, ts):
    with engine.begin() as conn:
        ...
        if operation == "insert" or operation == "update":
            all_keys_after = {
                "aircraft_code_seat_no": after["aircraft_code_seat_no"],
                "time": ts,
                "fare_conditions": after["fare_conditions"],
                "link_hash": concat_hash([after["aircraft_code_seat_no"], after["aircraft_code"]]),
                "aircraft_code": after["aircraft_code"],
            }
        elif operation == "delete":
            all_keys_before = {
                "aircraft_code_seat_no": before["aircraft_code_seat_no"],
                "time": ts,
                "fare_conditions": before["fare_conditions"],
                "link_hash": concat_hash([before["aircraft_code_seat_no"], before["aircraft_code"]]),
                "aircraft_code": before["aircraft_code"]
            }
    ...
    with engine.begin() as conn:
        ...
        if operation == "insert" or operation == "update":
            all_keys_after = {
                ...
                "ticket_no": after["ticket_no"],
                "link_hash_ticket_flights_boarding_passes": concat_hash([str(after["ticket_no"]) + "," + str(after["flight_id"]), str(after["ticket_no"]) + "," + str(after["flight_id"])]),
            ...
            all_keys_before = {
                ...
                "ticket_no": before["ticket_no"],
                "link_hash_ticket_flights_boarding_passes": concat_hash([str(before["ticket_no"]) + "," + str(before["flight_id"]), str(before["ticket_no"]) + "," + str(before["flight_id"])]),
        ...
        if operation == "insert":
            ...
            # теперь добавляем линк ticket_flights c boarding_passes
            conn.execute(
                text(insert_into_link(
                    "Link_ticket_flights_boarding_passes", "ticket_flights", "link_ticket_flights_boarding_passes_hash_key", "hub_ticket_flights_hash_key", 
                    "hub_boarding_passes_hash_key", "link_hash_ticket_flights_boarding_passes", "ticket_no_flight_id", "ticket_no_flight_id"
                )), 
                all_keys_after
            )
            ...
            conn.execute(text(
                update_query_in_update_operation(
                    "Link_ticket_flights_boarding_passes", 
                    "link_ticket_flights_boarding_passes_hash_key = ((:link_hash_ticket_flights_boarding_passes)::varchar)"
                )), all_keys_after
            )
